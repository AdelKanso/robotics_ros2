...
class ImageProjection(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameters(
            ...
            parameters=[
                ...
                ('camera.extrinsic_camera_calibration.offset_x', 0),
                ('camera.extrinsic_camera_calibration.offset_y', 0),
        ...
        self.offset_x = self.get_parameter('camera.extrinsic_camera_calibration.offset_x').value
        self.offset_y = self.get_parameter('camera.extrinsic_camera_calibration.offset_y').value
    ...
    def cbGetImageProjectionParam(self, params):
        for param in params:
            ...
            if param.name.endswith('offset_x'):
                self.offset_x = param.value
            if param.name.endswith('offset_y'):
    ...
    def cbImageProjection(self, msg_img):
        ...
        cx = w_img // 2 + self.offset_x
        cy = h_img // 2 + self.offset_y
